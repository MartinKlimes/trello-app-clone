...
const onSubmit = async () => {
  try {
    const res = await login({ identifier: formData.identifier, password: formData.password })
    navigateTo('/')
  } catch (e) {
    e ? err.value = e : null
  }
              ...
              <form class="space-y-4 md:space-y-6" @submit.prevent="onSubmit">
                ...
                {{ err.error.message }}
                  ...
                  <div>
                      ...
                      <input v-model="formData.identifier" type="text" name="email" id="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="name@company.com" required="">
                  </div>
                  <div>
                  ...
                  <div class="flex items-center justify-between">
                      ...
                      <!-- <NuxtLink to="recovery" class="text-sm font-medium text-primary-600 hover:underline dark:text-primary-500">Forgot password?</NuxtLink> -->
                  </div>
